...
# 
# sppmaster <- dbConnect(odbc::odbc(), .connection_string = "Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=C:/OneDrive - Personal/OneDrive/BCSpeciesList/SpeciesTaxonomyMaster.accdb;")
...
#veg.dat2 <- lump_species(veg.dat2, lump)
...
#ksi <- c("TSUGMER", "LYSIAME", "OPLOHOR", "ATHYFIL", "SPHAGNUM", "CLADONIA")

ksi <- c("TSUGMER", "PICEENE", "TSUGHET")#,, "THUJPLI")

tic()
###select units to run
su2 <- su
su2 <- su2 %>% filter(!grepl('CWHvh3|CWHwh1|CWHwh2|ICHvc', SiteUnit))###BGC specific|CWHms
...
vegsum.pairs <- do_pairwise(veg.dat2, su2, minimportance = 0.5, minconstancy = 60, noiseconstancy = 10, minplots = 5, 
                            covadj = .33, domcov = 10,
                            use.ksi = TRUE, ksi = ksi, ksi.value = 2, 
                            reduce.lifeform = TRUE, reduced.lifeforms = c(8, 9, 10, 11), reduction = 0.1 )
toc()
...
#review = 
xx <- vegsum.pairs %>% filter(Unit1 == "CWHds1_101a", Unit2 == "CWHds2_101a") %>% select(Unit1, Unit2, Species, diagnostic.potential.x, diagnostic.potential.y, diag.potential.tot, shared.diag, diag.points.x, diag.points.y,  sum.shared.diag, diag.points.x, diag.points.y, diag.tot, diag.ratio)
...
assoc <- cutree(h = .12, dendro_hc) %>% data.frame %>% rownames_to_column("SiteUnit") %>% rename(Assoc = ".") %>% mutate(Assoc = paste0("assoc-", Assoc))
alliance <- cutree(h = .20, dendro_hc) %>% data.frame %>% rownames_to_column("SiteUnit") %>% rename(Alliance = ".")%>% mutate(Alliance = paste0("all-", Alliance))
order <- cutree(h = .8, dendro_hc) %>% data.frame %>% rownames_to_column("SiteUnit") %>% rename(Order = ".")%>% mutate(Order = paste0("ord-", Order))
suborder <- cutree(h = .6, dendro_hc) %>% data.frame %>% rownames_to_column("SiteUnit") %>% rename(Suborder = ".")%>% mutate(Suborder = paste0("subord-", Suborder))
