...
su.phases <- vegsum.pairs %>% select(Unit1, Unit2, diff.ratio) %>% filter(endsWith(Unit1, c("a","b")), endsWith(Unit2, c( "b"))) %>% distinct()
...
su.similar <- vegsum.pairs %>% select(Unit1, Unit2, diff.ratio) %>% dplyr::filter(diff.ratio > .1) %>% filter(!Unit1 == Unit2)  %>%  distinct
...
```{r cluster analysis}
set.seed(1279)
dis.matrix <- as.data.frame(vegsum.pairs) %>% mutate(diss = 1-diff.ratio) %>% mutate(Unit1 = as.character(Unit1)) %>%  select(Unit1,Unit2, diss) %>% distinct %>%
...
```{r examine pairs}
# xx <- vegsum.pairs  %>% select(Unit1, Unit2, Species, diagnostic.potential.x, diagnostic.potential.y, diag.potential.tot, shared.diag, diag.points.x, diag.points.y,  sum.shared.diag, diag.points.x, diag.points.y, diag.tot, diff.ratio) %>% filter(diag.ratio>.7)# %>% filter(Unit1 %in% c("MHmm2_101"), Unit2 %in% c("MHmm2_110"))
# 
...
clst.units <- left_join(class, order, by = "SiteUnit") %>%
                    ...
                    left_join(su) %>% select(-SiteUnit) %>%  rename(SiteUnit = SiteUnit.orig)  %>%
...
new_su <- left_join(su2, su)%>% select(PlotNumber, SiteUnit.orig) %>% rename(SiteUnit = SiteUnit.orig)
