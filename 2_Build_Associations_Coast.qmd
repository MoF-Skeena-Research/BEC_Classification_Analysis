...
group.to.test <- "working_Assoc"## uses data for this field to initially group siteunits where applicablle
...
```{r merge very similar, echo=FALSE}
...
# 
# subass <- su_working2 %>% select(SiteUnit = SiteUnit.orig, tempSubAss_Code) %>%
...
su_working2 <- su_working2 %>%
  mutate(SiteUnit = coalesce(
    ...
    as.character(working_Assoc), # previously defined associations
...
# author = "Coastal_Assoc2"
# 
#  veg_all <- create_veg_sum_all(
#     vdat = veg.dat2,
#     siteUnits = su_working2 ,
#     minconstancy = 50,
#     noiseconstancy = 25,
#     strata.by = "Lifeform"
#   )
#  veg_all$Group = "All"
#  veg_all <- veg_all %>% select(SiteUnit, Species, Lifeform, MeanCov, Constancy, nplots, Group)
# 
# usa_units2 <- usa_units %>%
#   mutate(
#     Lifeform = 0,
#     Group = "All"
#   ) %>%
#   select(SiteUnit, Species, Lifeform, MeanCov, Constancy, nplots, Group)# %>%
#   # left_join(
#   #   taxon.lifeform %>% select(Code, Lifeform),
#   #   by = c("Species" = "Code")
#   # ) %>%
#   # mutate(
#   #   Lifeform = ifelse(is.na(Lifeform.y), Lifeform.x, Lifeform.y)
#   # ) %>%
#   # select(-Lifeform.x, -Lifeform.y)
# 
#  veg.comb <- rbind(veg_all, usa_units2) %>% distinct %>% drop_na %>% mutate(Lifeform = 0, Group= "All") %>%
#    filter(!Species %in% taxon.exotic)
# veg_all_tbl <- format_veg_table(vsum = veg.comb, spp = taxon.lifeform)
# 
# 
# vegsum.wbk <- createWorkbook()
# 
#   addWorksheet(vegsum.wbk, "All")
# 
#   # Write combined table
#   writeData(vegsum.wbk, sheet = "All", veg_all_tbl, startCol = 1, startRow = 1)
# 
# saveWorkbook(
#   vegsum.wbk,
#   paste0("./vegsum.tables/Compare_tables_", author, ".xlsx"),
#   overwrite = TRUE
# )
...
```{r cluster analysis with existing units, echo=FALSE, results= 'hide'}
source("../BEC_R_working/_functions/_bec_dist_matrix.R")
fig.show='hide'
#--------- do for associations
dendro_hc <- create_dendro_all(vegsum.pairs)
...
working.hier <- su_working2 %>% select(SiteUnit.base, generated_SubAss, working_Assoc) %>% distinct
working.hier2 <- working.hier %>%
  ...
  left_join(cluster_dt, by = c("working_Assoc" = "SiteUnit")) %>%
...
cluster_update_assoc <- merge(cluster_dt_assoc, su.anal1, by = c("SiteUnit"), all.x = TRUE) %>% select(SS_Label = SiteUnit.orig,  tempAssoc) %>% distinct %>% drop_na
...
working.hier3 <- su_working2 %>% select(SiteUnit.orig, SiteUnit.base, generated_SubAss, working_Assoc) %>% distinct %>% 
...
cluster_update2 <- left_join(cluster_dt_alli, working.hier3, by = c("SiteUnit" = "highest")) %>% select(SiteUnit, SU_Label = SiteUnit.orig,  generated_Alliance) %>% distinct 
...
cluster_update <- merge(cluster_dt, su.anal1, by = c("SiteUnit"), all.x = TRUE) %>% select(SiteUnit, tempAssoc) %>% distinct %>% drop_na

su_all_sim <- ss_similar %>%
  left_join(
    cluster_update %>% rename(Unit1 = SiteUnit, tempAssoc1 = tempAssoc),
    by = "Unit1"
  ) %>%
  left_join(
    cluster_update %>% rename(Unit2 = SiteUnit, tempAssoc2 = tempAssoc),
    by = "Unit2"
  ) %>%
  mutate(
...
su.anal.new <- su_working2 %>% select(PlotNumber, SiteUnit = SiteUnit.orig) %>%
  mutate(SiteUnit = str_replace(SiteUnit, "[abc]$", "")) %>% left_join(BEC.hier.temp, by = c("SiteUnit"= "SU_Label"))
...
assoc_edatope <- plot.edatope %>%
  ...
  select(SiteUnit, aSMR, SNR, groups = generated_Assoc) %>%
...
# veg.dat2 <- combine_taxa_strata(vegdata = vegdata, lump, use.subtaxa = FALSE) %>% filter(!Species == "UNKNOWN")
# vegsum.wbk <- createWorkbook()
# su.anal.new <- su.anal.new %>% mutate(groups = generated_Alliance)
# # Loop over each cluster
# for (g in sort(unique(su.anal.new$groups))) {
# 
#   # SiteUnits in this group
...
## Rerun cluster with generated Assocs to generate Alliances

Use updated generated Assoc from BECdb which include working assocs from previous step and initial predefined groups to rerun the pairwise analysis and cluster analysis. This will help to group remaining singletons with low diagnostic potential or few plots into working associations for review and editing.
...
# dbDisconnect(becdb)
# 
# group.to.test <- "generated_Assoc"
# # anal.set <- anal.set
# su.anal.new <- left_join (su_working2, BEC.hier.temp , by = c("SiteUnit.orig" = "SU_Label"))
# 
# 
# #------------------------------------------------------------
# # 4. Compute anal.test by walking down the hierarchy
...
## Now run another cluster with generated associations where applicable
...
Use generated associations and remaining singletons with low diagnostic potential or few plots into working associations for review and editing.

```{r}
# source("../BEC_R_working/_functions/_bec_dist_matrix.R")
# single.units <- draw_dendro_split(unit.compare = vegsum.pairs.new, cut.level=.4)
# ## write generated_Alliance to BECdb
```

```{r cluster analysis alliance, echo=FALSE, results= 'hide'}
# source("../BEC_R_working/_functions/_bec_dist_matrix.R")
# fig.show='hide'
# 
# dendro_hc <- create_dendro_all(vegsum.pairs.new)
# clusters <- cutree(dendro_hc, h = 0.4)
# cluster_dt <- data.table(
#   SiteUnit = names(clusters),
#   generated_alliance = paste0("coast_generated_alliance", clusters)
# )
# 
# ## merge back to retrieve SiteUnit for all and the coalesce.
# working.hier <- su_working2 %>% select(SiteUnit.base, generated_SubAss, generated_Assoc, generated_Alliance) %>% distinct
# working.hier2 <- working.hier %>%
#   left_join(cluster_dt, by = c("SiteUnit.base" = "SiteUnit")) %>%
#   rename(tempAssoc_base = tempAssoc) %>%
#   left_join(cluster_dt, by = c("generated_SubAss" = "SiteUnit")) %>%
...
# 
# # Filter cluster_dt to only those units with > 1 unit in clusters
# cluster_dt <- temp_assoc[tempAssoc %in% valid_clusters]
# 
# # setDT(su.anal1)
# # 
# # # If joining new units to existing unit save the original association name instead to the cluster name
...
# 
# su.anal.new <- su_working2 %>% select(PlotNumber, SiteUnit.orig) %>% left_join(BEC.hier.temp, by = c("SiteUnit.orig" = "SU_Label")) %>% rename(SiteUnit = SiteUnit.orig)
# #su.anal.new <- left_join(su_working3)
# 
# # anal.set <- anal.set
...
# )
# 
# groups <- unique(su.anal.new[[group.to.test]])
# 
# 
# vegsum.wbk <- createWorkbook()
# 
...
#   # ------------------------------------------------------------
# # su_order_df has SiteUnit.orig and tempAssoc_Code in the right order
# su_group_ord <- su_group %>% select(SiteUnit, generated_Assoc) %>%
#   dplyr::arrange(generated_Assoc, SiteUnit) %>% distinct %>% replace_na(list(generated_Assoc = "No_Assoc")) # replace NA with "No_Assoc" for ordering
# assoc_row_tbl <- su_group_ord %>%
#   tidyr::pivot_wider(
#     names_from  = SiteUnit,
...
____OLD CODE____________________________
...
# 
# 
# su_working3 <- su_working2 %>% select(PlotNumber, SiteUnit.orig) %>% left_join(BEC.hier2, by = c("SiteUnit.orig" = "SU_Label"))
# su.anal.new <- left_join(su_working3)
# 
# vegdata <- readRDS(file.path(data.path,"BEC_veg_data/Reports_BECMaster_Veg.rds"))
# veg.dat2 <- combine_taxa_strata(vegdata = vegdata, lump, use.subtaxa = FALSE) %>% filter(!Species == "UNKNOWN")
...
Set the cut level to 40% dissimilarity to generate preliminary alliances for review and editing. This will be used to review the proposed alliances and make decisions about which site units should be merged into associations, which should be separated into new associations, and which should be left as singletons. The vegetation summary tables will also be used to identify potential diagnostic species for each alliance and to compare the vegetation composition of different alliances.
...
#   SiteUnit = names(clusters),
#   tempAlliance = paste0("All_", clusters)) %>% 
#   left_join(BEC.hier, by = c("SiteUnit" = "working_Assoc")) %>% select(SS_Label, tempAlliance) %>% distinct %>% drop_na
# 
