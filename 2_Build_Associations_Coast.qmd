...
su <- su %>%
  filter(!grepl('101a.2|101b.2|low|-S|add|nudum|poor|yc|_W|_F|ys|moved|X', SiteUnit))
...
setDT(su)[merge, SiteUnit := mergedUnit, on = "SiteUnit"]
...
unit.compare <- vegsum.pairs %>%
  select(Unit1, Unit2, BEC.sim.min, BEC.sim.mean ,diff.ratio.x, diff.ratio.y, diff.ratio, nplots.x, unit.diag.sum.x) %>% filter(nplots.x>0) %>% 
 distinct()
unit.compare$bgc1 <- stringr::word(unit.compare$Unit1, 1, sep = "\\_")
unit.compare$bgc2 <- stringr::word(unit.compare$Unit2, 1, sep = "\\_")

yy <- vegsum.pairs %>% 
  select(Unit1, Unit2, Species, diagnostic.potential.x, diagnostic.potential.y, diff.pts.x, diff.pts.y, sum.shared.diag,diff.tot.x, diff.tot.y,  diff.ratio.x, diff.ratio.y, BEC.sim.min,BEC.sim.mean, diff.ratio.min) %>% 
  filter(Unit1 == "CWHws2_101", Unit2 == "CWHws2_110")
db <- "D:/BC_Correlation2_Vpro_2023/CoastGuide_Forested.accdb"
coast.units <- combined_su(db) %>% select(SiteUnit) %>% distinct
 dis.matrix <- bec_dist_matrix(unit.compare) %>% as.data.frame %>% rownames_to_column("SiteUnit")
...
assocs <- cluster_groups(unit.compare, cut.level = 0.2, minclus = 2, group.label = "Association")
assocs <- full_join(assocs, merge ) %>% filter(!SiteUnit %in% mergedUnit) %>% mutate(Association = ifelse(!is.na(mergedUnit), mergedUnit, Association)) %>% mutate(mergedUnit = ifelse(is.na(mergedUnit), SiteUnit, mergedUnit))
alliances <- cluster_groups(unit.compare, cut.level = 0.4, minclus = 2, group.label = "Alliance")
alliances  <- left_join(alliances , assocs, by = c("SiteUnit" = "mergedUnit"), keep = TRUE)
orders <- cluster_groups(unit.compare, cut.level = 0.6, minclus = 2, group.label = "Order")
orders  <- left_join(orders, alliances, by = c("SiteUnit" = "mergedUnit"), keep = TRUE) #%>% select(SiteUnit, Alliance)
...
coast.units <- left_join(su.coast, orders, by = c("SiteUnit" = "SiteUnit.y")) %>% select(PlotNumber, SiteUnit, Association, Alliance, Order)
...
coast.units <- rbind(coast.units, interior.units)

fwrite(coast.units, "./clean_data/Association_Units_v1.csv")
