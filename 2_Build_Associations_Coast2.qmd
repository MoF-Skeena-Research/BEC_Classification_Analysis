...
cluster_by_spp_group <- function(zonal.compare,
  ...
  membership_df <- do.call(rbind, all_memberships) %>% mutate(Association = NA)
...
xx <- fread("zonal_cluster_memberships.csv")## modify to hard code site series association membership
```


Step to hard code the merged site units from the review process.
```{r merge reviewed singles, echo=FALSE}

assign_cluster_names <- function(su.tab,
                                 cluster.file,
                                 siteunit_col = "OriginalUnit",
  ...
  membership <- membership %>%
    dplyr::rename(SiteUnit = all_of(siteunit_col),
                  NewName = all_of(new_name_col))

  # Join and update SiteUnit
  updated_data <- su.tab %>%
    dplyr::left_join(membership, by = "SiteUnit") %>%
    dplyr::mutate(SiteUnit = ifelse(!is.na(NewName), NewName, SiteUnit)) %>%
    dplyr::select(-NewName)

  return(updated_data)
}

su_updated <- assign_cluster_names(su.tab = su2, cluster.file = unit_lineage,
                                 siteunit_col = "OriginalUnit",
                                 new_name_col = "Association",
                                   use_last_step = TRUE)

# assign_cluster_names <- function(su.tab, cluster.file, siteunit_col = "SiteUnit", new_name_col = "Association") {
#   # Read in the updated cluster membership table
#   membership <- fread(cluster.file)
# 
#   # Ensure the relevant columns exist
#   if (!all(c(siteunit_col, new_name_col) %in% colnames(membership))) {
#     stop("Required columns not found in membership file.")
#   }
# 
#   # Rename columns for join clarity
#   colnames(membership)[colnames(membership) == siteunit_col] <- "SiteUnit"
#   colnames(membership)[colnames(membership) == new_name_col] <- "NewName"
# 
#   # Join and replace original SiteUnit with the new cluster name
#   updated_data <- su.tab %>%
#     left_join(membership, by = "SiteUnit") %>%
#     mutate(SiteUnit = ifelse(!is.na(NewName), NewName, SiteUnit)) %>%
#     select(-NewName)
# 
#   return(updated_data)
# }
# Load your original plot-level data (e.g., vegsum2 or similar)

# Apply the updated names
su_updated <- assign_cluster_names(su.tab = su2, cluster.file = "./hierarchy/LMH77_zonal_assoc_memberships.csv")
...
all_results <- cluster_by_spp_group(
  unit.compare,
  anal.groups = NULL,
  cut.level = 0.5,
  combine.level = 0.15,
  ...
  include.few.plots = FALSE,
  few.plots.threshold = 4,
  output.file = "cluster_memberships2.csv"
)

xx <- fread("cluster_memberships2.csv") %>% group_by(Cluster, Spp.group) %>%
  mutate(n_SiteUnits = n_distinct(SiteUnit)) %>% ungroup %>% filter(n_SiteUnits > 1) %>%
  arrange(desc(n_SiteUnits)) %>% left_join(anal.groups, by = c("SiteUnit" = "SiteUnit")) %>%
fwrite(xx, "cluster_memberships2.csv")

# Need to combine the zonal groups with the new groups

# yy <- fread("./hierarchy/LMH77_zonal_assoc_memberships.csv") %>% mutate(Cluster = NA) %>% 
#   bind_rows(xx) %>% mutate(Cluster= ifelse(is.na(Cluster), Association, Cluster)) %>% left_join(
#   group_by(Cluster) %>%
#   mutate(Association = ifelse(!is.na(Association), Association, NA)) %>%
#   fill(Association, .direction = "downup") %>%# filter(!SiteUnit == Association) %>% 
#   ungroup()



su_updated2 <- assign_cluster_names(su.tab = su2, cluster.file = "./hierarchy/LMH77_zonal_assoc_memberships.csv")
...
```{r}
  ...
  zonal.compare = unit.compare; combine.level = 0.1;  max.steps = 2;  stop.no.change = TRUE;  verbose = TRUE; similarity_col = "BEC.sim.min"; unit1_col = "Unit1"; unit2_col = "Unit2"; 
