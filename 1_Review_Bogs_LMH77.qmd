...
su <- dbReadTable(master_su, "Coast_Bogs_Final_SU")
...
su <- su %>%
  filter(!str_detect(SiteUnit, "poor|low|-S|add|nudum|browsed|X|!|support|unplaced|\\$$")) %>%
...
su <- su %>% mutate(SiteUnit = ifelse(SiteUnit %in% c("Wb53.2", "Wb53.3"), "Wb53", SiteUnit)) 
unit.choose <- su %>% pull(SiteUnit)
table.name1 <- "Bogs1"
unit.choose <- c("Wb50", "Wb51", "Wb52", "Wb53", "Wb54", "Wb55.1", "Wb55.2", "Wb56", "Wb57", "Wb58")
su.choose <- su %>% filter(SiteUnit %in% unit.choose)
vegSum <- create_veg_sum_all(vdat = veg.dat2, siteUnits = su.choose, minimportance = 0, minconstancy = 50, noiseconstancy = 25, strata.by = "Lifeform")
##determine order of species by unit
indic.order <- build_species_ordering(vdat = veg.dat2, vsum = vegSum, siteUnits = su.choose)
veg.sum.table1 <- format_veg_table2(vsum = vegSum, spp = taxon.lifeform)#, type="All"
create_VGS_table(veg.sum.table1, table.name = table.name1)

# table.name2 <- "Bogs2"
# unit.choose <- c( "Wb52", "Wb53.1", "Wb53.2", "Wb56")
# su.choose <- su %>% filter(SiteUnit %in% unit.choose)
# vegSum <- create_veg_sum_all(vdat = veg.dat2, siteUnits = su.choose, minimportance = 0, minconstancy = 50, noiseconstancy = 25, strata.by = "Lifeform")
# ##determine order of species by unit
# indic.order <- build_species_ordering(vdat = veg.dat2, vsum = vegSum,  siteUnits = su.choose)
# veg.sum.table2 <- format_veg_table2(vsum = vegSum, spp = taxon.lifeform)#, type="All"
# create_VGS_table(veg.sum.table2, table.name = table.name2)
 
                  
#  unit.choose <- c("Bb13 Rumefue", "Bbd01 Poa_mac", "Bb_Festrub", "Bb_Calysol","Bb_Grinsti","Br Festrub","HwSs-Gaulsha","HwSs-Kindpra")  
# su.choose <- su %>% filter(SiteUnit %in% unit.choose)
# table.name3 <- "Beach3"
...
saveWorkbook(vegsum.wbk, "./vegsum.tables/LMH77_Bogs_table.xlsx", overwrite = TRUE)
