...
SUTab <- fread("./clean_tabs/BECv13Analysis_Forest_17Sept2021_SU.txt")
...
class.dat <-
  SS_good  %>% dplyr::select(SiteUnit, Species, MeanCov) %>% 
  pivot_wider(id_cols = c(SiteUnit),
    names_from = Species,
    values_from = MeanCov) %>% 
...
```{r predict all v13 from original hierarchy in v12}
load("./rFmodels/BECv12_Suborders_rFmodel_updated.Rdata")
BEC_new <- class.dat
...
newcols <- model_vars[!model_vars$rowname %in% BGC_new.var,] %>% select(1)
...
SUhier <- fread("./inputs/Test3_Hierarchy.csv") %>% as.data.frame() ## import orginal tree for updating
...
SUhier2 <- left_join(SUhier, newBEC3) %>% mutate(Parent = ifelse(!is.na(Parentnew), Parentnew, Parent)) %>% 
            select(-Parentnew) %>% select(ID, Name, Parent, Level)
SUhier2$Parent <- ifelse(SUhier2$Parent == 1, "", SUhier2$Parent) %>% as.integer
###write out to Vpro format to review
fwrite(SUhier2, "./outputs/UpdatedVPROHierarchyv13_updated.csv")
```
