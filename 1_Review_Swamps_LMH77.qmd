...
su <- dbReadTable(master_su, "Coast_Swamps_Final_SU")
...
table.name1 <- "RichSkunk"
 unit.choose <- c( "Ws52", "Ws53", "Ws56.1", "Ws56.2", "Ws56.3",  "Ws57", "Ws59", "Ws63")
...
table.name2 <- "PoorSkunk_Sedge"
 unit.choose <- c("Ws54.1", "Ws54.2", "Ws55","Ws58.1", "Ws58.2", "Ws60","Ws61", "Ws62")
 su.choose <- su %>% filter(SiteUnit %in% unit.choose)
vegSum <- create_veg_sum_all(vdat = veg.dat2, siteUnits = su.choose, minimportance = 0, minconstancy = 50, noiseconstancy = 25)
##determine order of species by unit
indic.order <- build_species_ordering_all(vdat = veg.dat2, vsum = vegSum, code.lump=lump, siteUnits = su.choose)
...
# table.name3 <- "Non-forestSwamp"
#  unit.choose <- c("Ws50.1", "Ws50.2", "Ws51.1", "Ws51.2")
#  su.choose <- su %>% filter(SiteUnit %in% unit.choose)
# vegSum <- create_veg_sum_all(vdat = veg.dat2, siteUnits = su.choose, minimportance = 0, minconstancy = 50, noiseconstancy = 25)
# ##determine order of species by unit
# indic.order <- build_species_ordering_all(vdat = veg.dat2, vsum = vegSum, code.lump=lump, siteUnits = su.choose)
...
saveWorkbook(vegsum.wbk, "./vegsum.tables/LMH77_Swamp_Associations.xlsx", overwrite = TRUE)
