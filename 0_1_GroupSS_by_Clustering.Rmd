...
SS_reduced_list <- unique(veg_anal$SiteUnit) %>% as.data.frame %>% rename(SS = 1)
...
minimum.iso1 <- test_jac.list %>% group_by(iso1) %>%  slice_min(order_by = dist) %>% filter(dist<.2) %>% select(iso1) %>% rename(SS = iso1)
minimum.iso2 <- test_jac.list %>% group_by(iso2) %>%  slice_min(order_by = dist) %>% filter(dist<.2) %>% select(iso2) %>% rename(SS = iso2)
SS_w_neighbours <- rbind(minimum.iso1,  minimum.iso2) %>% unique
SS_no_neighbours <- anti_join(SS_reduced_list, SS_w_neighbours)
fwrite(test_jac.tab, "./outputs/dissim_matrix.csv")
...
Hier_SU <- Hier_SU %>% select(-assocs)
assoc_SU2 <- assoc_SU2 %>% select(SiteUnit, assocs)
