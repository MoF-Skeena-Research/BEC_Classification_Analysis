...
db <- "D:/BC_Correlation2_Vpro_2023/CoastalGuide_NonForested.accdb"
...
su <- su %>%
  filter(!grepl('poor|low|-S|add|nudum|browsed', SiteUnit)) %>% select(-SiteUnit.orig) %>% arrange(SiteUnit)
...
#for (bgc.choose in bgc.unique){
  create_dendro_all(unit.compare, threshold.low = .07, threshold.high = .18)
#}
```

```{r veg guide summary table}
VGS_table <- function(veg.sum.table, table.name = ""){
...
table.name <- "Skunkcabbage"
vegSum <- create_veg_sum_all(vdat = veg.dat2, siteUnits = su, minimportance = 1)
##determine order of species by unit
indic.order <- build_species_ordering_all(vdat = veg.dat2, vsum = vegSum, code.lump=lump, siteUnits = su)
veg.sum.table <- format_veg_table2(vsum = vegSum, spp = taxon.lifeform)
VGS_table(veg.sum.table, table.name = "Skunkcabbage")
vegsum.wbk <- createWorkbook()
openxlsx::addWorksheet(vegsum.wbk, sheetName = table.name)
openxlsx::writeData(vegsum.wbk, sheet = table.name, veg.sum.table)
saveWorkbook(vegsum.wbk, "./vegsum.tables/LMH77_draft_skunkcabbage_table.xlsx", overwrite = TRUE)
```
