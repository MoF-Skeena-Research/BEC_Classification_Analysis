...
3.  Comparison of site series within each BGC for sufficient site unit differentiation
...
Research Baselga framework (based on pres-abs data) which partitions dissimilarity into species replacement (different species) versus nestedness (missing species in one unit).
...
source("../BEC_R_working/_functions/_diagram_diagnostic_points.R")

```

```{r}
p <- diagram_diagnostic_points(outfile = "Diagnostic_Potential_Scoring_System.png")
