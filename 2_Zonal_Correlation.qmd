...
bgcv13 <- bgcv12 %>% filter(is.na(BGCVersionRetired)) %>% select(BGC_NoSpace, SiteClassificationStatus)
```
...
su11 <- combined_su(db)
# db <- "D:/BC_Correlation2_Vpro_2023/working/Lillooet_FG_draft.accdb"
db <- "D:/OneDrive - Government of BC/Omineca BEC/Robson Valley/RobsonValley_SU_working.accdb"
su12 <- combined_su(db)
...
#|
su <- su %>%   filter(grepl('01', SiteUnit)) 
su <- su %>% filter(grepl(zone.choose, bgc))
SU <- su %>% select(PlotNumber, SiteUnit)
###write combined SU to database
master_su <- dbConnect(odbc::odbc(), .connection_string = "Driver={Microsoft Access Driver (*.mdb, *.accdb)};
DBQ=D:/BC_Correlation2_Vpro_2023/Zonal_Comparisons.accdb;")
dbWriteTable(master_su, paste0("All_", zone.choose, "_zonals_SU"),  SU, overwrite = TRUE, batch_rows = 1)
...
#su <- su %>% filter(grepl(zone.choose, SiteUnit))# %>%  mutate(bgc = ifelse(grepl('101.2', SiteUnit), paste0(bgc, "var2"), bgc))###BGC specific|CWHms
...
key.site.indicators <- c("CALLNOO","PICESIT","PSEUMEN", 
...
xx <- vegsum.pairs %>% filter(Unit1 == "ESSFwh4" | Unit2 == "ESSFwh4") %>% select(Unit1, Unit2, BEC.sim.min, BEC.sim.mean, BEC.sim.max) %>% distinct
...
xx <- draw_dendro_split(unit.compare = vegsum.pairs, cut.level=.3)
xx
```

```{r add cut level as group}
add_cluster_membership <- function(unit.compare, cut.level = .3){
...
groups <- add_cluster_membership(vegsum.pairs, cut.level = .3)
