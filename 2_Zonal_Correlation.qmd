...
lump <- dbReadTable(veglump, "CorrelationII_Zonal_Lump")
...
#su9 <- combined_su(db)
db <- "D:/BC_Correlation2_Vpro_2023/TOR_FG_draft.accdb"
su10 <- combined_su(db)

su <- rbind(su1, su2, su3, su4, su5, su6, su7, su8,  su10) %>%
...
su <- su %>%
  filter(!grepl('101a.2|101b.2|low|-S|add|nudum|poor|yc|_W|_F|ys|moved|X|support|101b|Y', SiteUnit)) %>% distinct
...
#su <- su %>% filter(grepl('ICH|ESSF', SiteUnit))###BGC specific|CWHms
su <- su %>% filter(grepl('ESSF', SiteUnit))###BGC specific|CWHms
...
reduced.exceptions <- c("SPHAGNUM", "CLADONIA", "CLADINA", "RACOMITR", "NEOORTHO")
...
fwrite(su.reduced, "./outputs/combined_SU_zonal_ESSF.csv")
...
saveWorkbook(vegsum.wbk, paste0("./vegsum.tables/ESSF_draft_veg_tables_", author, ".xlsx"), overwrite = TRUE)
