...
veg.dat2 <- lump_species(vegdata = veg.dat2, lump, use.subtaxa = TRUE) %>% filter(!Species == "UNKNOWN")
...
su <- su %>%
  filter(!grepl('101a.2|101b.2|low|-S|add|nudum|poor|yc|_W|_F|ys|moved|X|support|101b', SiteUnit)) %>% distinct
...
veg.dat2 <- lump_species2(vegdata = vegdata, lump, use.subtaxa = FALSE) %>% filter(!Species == "UNKNOWN")
...
saveWorkbook(vegsum.wbk, paste0("./vegsum.tables/ICH_draft_veg_tables_", author, ".xlsx"), overwrite = TRUE)
