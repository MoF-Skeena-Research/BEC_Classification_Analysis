...
```{r set folders, include=FALSE}
load("./clean_data/Analysis_BECMaster_Veg.RData")
...
veg_anal.tree <- create_su_vegdata(vegdata.tree, su) %>% create_analysis_vegsum (importance = 0.5, constancy = 60, minplots = 5, covadj = 1)
veg_anal.under <- create_su_vegdata(vegdata.understorey, su) %>% create_analysis_vegsum (importance = 0.5, constancy = 60, minplots = 5, covadj = .5)
veg_anal.moss <- create_su_vegdata(vegdata.moss, su) %>% create_analysis_vegsum (importance = 0.5, constancy = 60, minplots = 5, covadj = .1)
...
test_jac <- proxy::dist(veg.dat, method="eJaccard", diag=FALSE, upper = FALSE, pairwise = FALSE) ## ejaccard and philentropy jaccard are real-data versions extended jaccard opr taimoto
