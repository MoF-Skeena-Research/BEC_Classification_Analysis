...
```{r set folders, include=FALSE}
veg.dat <- readRDS("./clean_data/Analysis_BECMaster_Veg.rds")## generated from 0_0 Create Veg Analsysi Set.rmd
...
### for working on a select set of site series
# working_SU = "Coast_Skunkcabbage"
# working_su <- dbConnect(odbc::odbc(), .connection_string = paste0("Driver={Microsoft Access Driver (*.mdb, *.accdb)};  DBQ=D:/BC_Correlation2_Vpro_2023/working/", working_SU,".accdb;"))
...
#covadj exponent set based on Tichy range. max values for 100%const-100%cover is 15% for .6 and 4% for .3
veg_anal.tree <- create_su_vegdata(vegdata.tree, su) %>% 
          create_analysis_vegsum (minimportance = 1, minconstancy = 60, noiseconstancy = 20, minplots = 5, covadj = .6)
veg_anal.under <- create_su_vegdata(vegdata.understorey, su) %>%
          create_analysis_vegsum (minimportance = 1, minconstancy = 60, noiseconstancy = 20, minplots = 5, covadj = 0.6)
veg_anal.moss <- create_su_vegdata(vegdata.moss, su) %>%
          create_analysis_vegsum (minimportance = 1, minconstancy = 60, noiseconstancy = 20, minplots = 5, covadj = .3)
veg_anal <- rbind(veg_anal.tree, veg_anal.under, veg_anal.moss)
veg.anal.mtrx <-
  veg_anal.tree %>% dplyr::select(SiteUnit, Species, spp_importance) %>% matrify()
test_jac <- vegan::vegdist(as.data.frame(veg.anal.mtrx))
test_jac <- proxy::dist(veg.anal.mtrx, method="eJaccard", diag=FALSE, upper = FALSE, pairwise = FALSE)
# test_jac <- proxy::as.simil(test_jac) %>% as.matrix %>% as.data.frame
require(Matrix)
require(wordspace)
# veg.anal.mtrx <- Matrix(veg.anal.mtrx)
# test_jac <- dist.matrix(veg.anal.mtrx, method = "cosine", p = 2, 
...
test_jac <- proxy::dist(veg.anal.mtrx, method="eJaccard", diag=FALSE, upper = FALSE, pairwise = TRUE) ## ejaccard and philentropy jaccard are real-data versions extended jaccard opr taimoto
