---
title: "Review of North Thompson Draft Site Series by BGC"
...
source('../BEC_R_working/_functions/update_taxa.R')
...
xx <- veg.dat %>% filter(Species == "PECTBRE")
taxon.all <- read_sppmaster(database = "D:/OneDrive/BCSpeciesList/SpeciesTaxonomyMaster.accdb")
veg.dat <- update_taxa(veg.dat, taxon.all )
...
#db <- "D:/OneDrive - Government of BC/LMH81_NorthColumbia_Review_Nov2025/NorthThompson_FG_working.accdb"
su <- combined_su(db)
su.nf <- su %>%
  filter(grepl('_[[:alpha:]]', SiteUnit)) %>% select(-SiteUnit.orig) %>% arrange(SiteUnit)
#   filter(grepl('01', SiteUnit)) ###zonal specific
su <- su %>%
  filter(!grepl('support|poor|low|[$]|add|nudum|_[[:alpha:]]|X|omit|support|/x', SiteUnit)) %>%
  mutate(
    SiteUnit = sub("(101)\\.(1|2)$", "\\1", SiteUnit)   # convert 101.1 or 101.2 → 101
  ) %>%
...
key.site.indicators <- c("POPUTRI", "LYSIAME", "OPLOHOR", "ATHYFIL",  "EQUIARV", "GYMNDRY", "VALESIT", "CASSMER", "LUETPEC", "RHODGRO", "EMPENIG", "CALTLEP", "VACCSCO")#, "TSUGMER",  "TIARELLA", "DRYOEXP" )
...
```{r cluster analysis by bgc, echo=FALSE}
...
```{r build edatopic tables, echo=FALSE}
...
vegdata <- readRDS("./clean_data/Reports_BECMaster_Veg.rds")
vegdata <- update_taxa(vegdata, taxon.all )### named veg.d
...
for(bgc.choose in bgc.list){
vegSum <-
  create_veg_sum_bgc(vdat = veg.dat2, siteUnits = su3 , BGC = bgc.choose, minconstancy = 50, noiseconstancy = 25, minimportance = 0, strata.by = "Layer")
#yy <- vegSum %>% filter(SiteUnit == "ESSFvm1_111", Layer == "Herb")
##determine order of species by unit
indic.order <- build_species_ordering(vdat = veg.dat2, vsum = vegSum, siteUnits =su3, BGC = bgc.choose)
...
toc()
saveWorkbook(vegsum.wbk, paste0("./vegsum.tables/NThompson_FG_draft_veg_tables_", author, ".xlsx"), overwrite = TRUE)
