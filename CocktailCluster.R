...
while (i <= (n-2) ){
  ...
  phi.index <- designdist(t(vegmatrix2), method = "(a*d-b*c)/sqrt((a+c)*(b+d)*(a+b)*(c+d))",
                          terms = c("binary"), abcd = TRUE, alphagamma = FALSE, "phi")
...
write.csv(Plot.cluster,"./outputs/Plot_cluster_dune.csv",row.names=F)
...
phi.index <- designdist(t(dune), method = "(a*d-b*c)/sqrt((a+c)*(b+d)*(a+b)*(c+d))",
                        terms = c("binary"), abcd = T, alphagamma = FALSE, "phi")
