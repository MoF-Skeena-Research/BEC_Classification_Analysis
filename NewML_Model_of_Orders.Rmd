...
## A machine learning model of high-level hierarchical BEC Forest units
Create a ML model of Forested Classes, Orders, and Suborders using BECv11 units.
Predict membership of new BECv12 units and assign to suborders then rebuild ML model.
Compare similarity of tree composition within units to find outliers that represent new SubOrders
Predict membership of draft BECv13 units for test.
Create summaries and compare vegetation and climatic attributes of units. Identify climatic indicator groups.

## Pair-wise comparison of high constancy species to analyze site series within Suborders to create Associations/SubAssociations.

## Create Alliances based on indicator group creation and analysis.

## Build model to assign new units and unassigned plots

## Build for non-forested units. Classes based on major site level differences rather than climate and using major species groups (e.g. Hydrophytic Carex spp )
...
#vegDat_test <- vegDat2 %>% filter(Lifeform %in% trees )
...
Hier_update <- left_join(Hier.clean, UpdateClass, by  = "SiteSeries") %>% mutate(newClass = coalesce(.pred_class, Class)) %>% select(-Class, -.pred_class) %>% rename(Class = newClass) %>% select(ID, Region, Class, everything())
```
