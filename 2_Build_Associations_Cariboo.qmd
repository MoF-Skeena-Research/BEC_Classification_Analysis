...
key.site.indicators <- c("POPUTRI", "OPLOHOR", "GYMNDRY", "VALESIT", 
...
xx <- draw_dendro_split(unit.compare2, cut.level=.3)
...
assocs <- full_join(assocs, merge ) %>% filter(!SiteUnit %in% mergedUnit) %>% 
  ...
  mutate(mergedUnit = ifelse(is.na(mergedUnit), SiteUnit, mergedUnit)) %>% mutate(Association = paste0("assoc-", Association))
alliances <- cluster_groups(unit.compare, cut.level = 0.4, minclus = 0, group.label = "Alliance")
alliances  <- left_join(assocs, alliances ,  by = c("mergedUnit"="SiteUnit"), keep = TRUE) %>% select(Alliance, Association, SiteUnit.x, mergedUnit)%>% mutate(Alliance = paste0("all-", Alliance))
orders <- cluster_groups(unit.compare, cut.level = 0.8, minclus = 0, group.label = "Order")
orders  <- left_join(alliances, orders,  by = c("mergedUnit"="SiteUnit"), keep = TRUE) %>% select(-SiteUnit, Order, Alliance, Association, SiteUnit.x, mergedUnit)%>% mutate(Order = paste0("order-", Order))
classes <- cluster_groups(unit.compare, cut.level = .99, minclus = 0, group.label = "Class")
classes  <- left_join(orders,classes,  by = c("mergedUnit"="SiteUnit"), keep = TRUE) %>% select(SiteUnit.x, Association, Alliance, Order, Class)%>% mutate(Class = paste0("class-", Class))
...
coast.units.su <- left_join(classes, su3,  by = c("SiteUnit.x" = "SiteUnit.x")) %>% select(PlotNumber, SiteUnit.x, SiteUnit.orig.x, Association, Alliance, Order, Class, bgc) %>% rename(SiteUnit = SiteUnit.orig.x)
