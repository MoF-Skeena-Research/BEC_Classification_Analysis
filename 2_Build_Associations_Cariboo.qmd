...
# su <- combined_su(db)
# su <- su %>%
#   filter(!grepl('101a.2|101b.2|low|-S|add|nudum|poor|yc|_W|_F|ys|moved|X|unplaced', SiteUnit))  %>% arrange(SiteUnit)
# #   filter(grepl('01', SiteUnit)) ###zonal specific
# su2 <- su %>% select(-SiteUnit.orig) %>%
#   filter(grepl('SBS|IDF|MS', bgc))
...
su <- dbReadTable(master_su, "All_BGC12DEC2024_SU")
...
#key.site.indicators <- c("LYSIAME",   "ELLIPYR","ATHYFIL", "RUBUSPE",   "CASSMER", "LUETPEC", "POLYMUN", "STRUSPI", "RHODGRO", "EMPENIG")#,  "TIARELLA", "DRYOEXP" )
key.site.indicators <- c("POPUTRI", "OPLOHOR", "GYMNDRY", "VALESIT", "THALOCC", "CASSMER", "LUETPEC", "EMPENIG", "LYSIAME",  "ATHYFIL", "VERAVIR", 
                         "ACERGLA", "GEOCLIV", "CALARUB",## 
                         "RHODALB","EQUIARV", ## ESSF
                        "TSUGHET", "THUJPLI", "TSUGMER" ) 
...
vegsum.pairs <- do_pairwise(veg.dat2,
  su = su2, minimportance = 0, minconstancy = 50,
  noiseconstancy = 10,
  minplots = 1,  minor = 1,
...
toc()
# xx <- vegsum.pairs %>%
#   filter(startsWith (Unit1, "ESSFwc3"), BEC.sim.min >.80)
# xx2 <- vegsum.pairs %>%
#   filter(startsWith (Unit2, "ESSFwc3"), BEC.sim.min >.80)
# # , Unit2 == "CWHws2_112b")
# vegsum.pairs <- rbind(xx, xx2)
...
unit.simple <- unit.compare %>% filter(unit.diag.sum.x < 40)  %>% select(Unit1, unit.diag.sum.x) %>% distinct
...
singles.poor <- as.data.frame(xx) %>% filter(grepl('nplot|lowdiag', SiteUnit))
singles.good <- as.data.frame(xx) %>% filter(!grepl('nplot|lowdiag', SiteUnit))
```
...
```{r pull associations from dendrogram at cut.level}

assocs <- cluster_groups(unit.compare, cut.level = 0.2, minclus = 0, group.label = "Association")
assocs <- assocs %>% mutate(mergedUnit = NA) %>% filter(!SiteUnit %in% mergedUnit) %>%
  mutate(Association = ifelse(!is.na(mergedUnit), mergedUnit, Association)) %>%
  mutate(mergedUnit = ifelse(is.na(mergedUnit), SiteUnit, mergedUnit)) %>% mutate(Association = paste0("assoc-", Association)) %>% select(-SiteUnit)
alliances <- cluster_groups(unit.compare, cut.level = 0.4, minclus = 0, group.label = "Alliance")
alliances  <- left_join(assocs, alliances,  by = c("mergedUnit"="SiteUnit"), keep = TRUE) %>% select(Alliance, Association, mergedUnit)%>% mutate(Alliance = paste0("all-", Alliance))
orders <- cluster_groups(unit.compare, cut.level = 0.8, minclus = 0, group.label = "Order")
orders  <- left_join(alliances, orders,  by = c("mergedUnit"="SiteUnit"), keep = TRUE) %>% select(-SiteUnit, Order, Alliance, Association, mergedUnit)%>% mutate(Order = paste0("order-", Order))
classes <- cluster_groups(unit.compare, cut.level = .99, minclus = 0, group.label = "Class")
classes  <- left_join(orders,classes,  by = c("mergedUnit"="SiteUnit"), keep = TRUE) %>% select(mergedUnit, Association, Alliance, Order, Class)%>% mutate(Class = paste0("class-", Class))
su.temp <- su %>% select(PlotNumber,  SiteUnit)
su3 <- left_join(su.temp, su, by = c("PlotNumber", "SiteUnit"))
...
coast.units.su <- left_join(classes, su3,  by = c("mergedUnit" = "SiteUnit")) 

coast.units.su <- coast.units.su %>% dplyr::select(PlotNumber, mergedUnit, Association, Alliance, Order, Class, bgc) %>% rename(SiteUnit = mergedUnit)
...
fwrite(coast.units.su, "./clean_data/ESSFwc3Association_Units_v2.csv")
                                                                                                                         #left_join(su.coast, orders, by = c("SiteUnit" = "SiteUnit.y")) %>% select(PlotNumber, SiteUnit, Association, Alliance, Order)
```
...
author <- "test.assoc2"
assoc.list <- coast.units %>% filter(!is.na(Association)) %>% group_by(Association) %>% mutate(n=n()) %>% filter(n>=2) %>% distinct(Association) %>% pull(Association)

vegsum.wbk <- createWorkbook()
for(assoc.choose in assoc.list){
#assoc.choose = "assoc-2"
assoc <- coast.units.su %>% filter(Association %in% assoc.choose) %>% select(SiteUnit, Association) %>% mutate(SiteUnit = gsub(" ", "", SiteUnit)) %>% mutate(SiteUnit = str_replace(SiteUnit, "/", "_"))
su.assoc <- left_join(assoc, su2, by = "SiteUnit") %>%  mutate(SiteUnit = str_replace(SiteUnit, "b$|a$", "")) %>% distinct()
...
vsum_units <-vegSum %>% select(SiteUnit) %>% distinct %>% pull(SiteUnit)
su_units <- su.assoc %>% select(SiteUnit) %>% distinct %>% pull(SiteUnit)


indic.order <- build_species_ordering_hier(vdat = veg.dat.assoc, vsum = vegSum, code.lump=lump, siteUnits = su.assoc, hier.level = "Association")
...
saveWorkbook(vegsum.wbk, paste0("./vegsum.tables/ESSF_draft_assoc_tables_v4_", author, ".xlsx"), overwrite = TRUE)
...
#test <- fread("G:/BECv13_Hierarchy_Matrix.csv") %>% as.data.table
test <- coast.hier#[21:38]
