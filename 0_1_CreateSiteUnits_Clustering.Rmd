...
test_jac <- proxy::dist(test.data, method="ejaccard", diag=TRUE, upper = FALSE) ## ejaccard and philentropy jaccard are real-data versions extended jaccard opr taimoto
...
#                     rect = T, cex = .5, horiz = T) %>%    plot()
  # 
assoc_SU <- as.data.frame(assocs) %>% rownames_to_column("siteseries") %>% mutate(assocs = paste0("assoc-", assocs)) %>% arrange(desc(PlotNumber))
asspc
assoc_SU <- assoc_SU %>% left_join(veg
...
fwrite(assoc_SU, "./outputs/CoastForestAssocs_SU.csv")
```
