...
create_veg_sum <- function(vdat, siteUnits, minconstancy = 60, noiseconstancy = 10, strata.by = "Layer", BGC) {
  if (strata.by == "Layer") {
...
create_veg_sum_hier <- function(vdat, siteUnits, minconstancy = 60, noiseconstancy = 10,
                                strata.by = "Lifeform", cluster) {
  if (strata.by == "Layer") {
    vdat <- lump_species2(vdat, lump = lump)
  } else if (strata.by == "Lifeform") {
    vdat <- vdat <- lump_species(vdat, lump = lump)
