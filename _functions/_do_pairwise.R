...
do_pairwise <- function(veg.dat, su, minimportance = 0, minconstancy = 60,
  ...
  vegsum <- vegsum %>%
    rowwise() %>%
    mutate(constant_type = ifelse((Constancy >= minconstancy & MeanCov >= domcov), "cd",
  ...
  vegsum <- vegsum %>% 
    mutate(dd.potential = ifelse(MeanCov >= domcov, (MeanCov^(1 / 3)),
  ...
  if (isTRUE(reduce.lifeform)) {
    vegsum[,  diagnostic.potential := ifelse(Lifeform %in% reduced.lifeforms & !Species %in% reduced.exceptions, 
                          diagnostic.potential * 0.1, diagnostic.potential)
           ]
  }
 ...
 vegsum <- vegsum %>% dplyr::select(-ScientificName, -EnglishName, -Lifeform)
  ...
  vegsum.pairs[, `:=`(
    dd.points.x = fifelse((constant_type.x == "cd") & (cover2.diff >= 0.9), cover2.diff, 0),
    dd.points.y = fifelse((constant_type.y == "cd") & (cover2.diff <= -0.9), (0 - cover2.diff), 0)
  )][, `:=`(
