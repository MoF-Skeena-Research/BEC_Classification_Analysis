...
BECv13_SU <- fread("./clean_tabs/BECv13Analysis_Forest_17Sept2021_SU.txt", stringsAsFactors = FALSE)
BECv13_Hier <- fread("./clean_tabs/BEC13_HierarchyMatrixModelled_v1.csv")
...
#---------------
BECv13Hier.data <- left_join(BECv13_vegSum,BECv13_Hier)
...
Orders
Order.dat <- BECv13Hier.data %>% filter(Order == "ORDER Picegla-Hylospl (BWBS/SWB) (Sw)")
...
Spp.highconst <- Order.dat  %>%  group_by(Species) %>%
  summarise(max = max(Constancy, na.rm=TRUE)) %>% filter(max > constcut) %>% select(Species) 
...
Order.dat2 <- Order.dat2 %>% select(Order,SiteUnit, Species, MeanCov, Constancy)
...
Order_env <- left_join(Order_SU, BECv13_env, by = "PlotNumber") %>% filter(!MAT == -9999) %>% select(PlotNumber, SiteUnit,  CMD, DD5, DD1040, bFFP)
...
SS_envsum <- Order_env3[,.(MeanCov = sum(Cover, na.rm = TRUE)/nPlots[1], Constancy = (.N/nPlots[1])*100, nPlots = nPlots[1]), by = .(SiteUnit,Species)]
