...
vegDat2 <- fread("./inputs/BECMaster_VegR_clean.csv", data.table = FALSE)
sppMaster <- fread('./inputs/SpeciesMaster05Oct2019.csv') %>% filter(Codetype == "U")
...
##Import cleaned wide matrix form 0_0
...
fwrite(vegSum, './clean_tabs/BECv13SiteUnitSummary.csv')
```
...
covercut <- 0.1
treespp <- sppMaster[Lifeform %in% trees, ] %>% dplyr::select(Code)
...
Hier.units <- SUhier %>% dplyr::select(SiteUnit, Class, Order, Suborder) %>% distinct()
#Hier.units <- Hier_update%>% select(SiteUnit, Class, Order, Suborder)
...
class.dat <-
  Hier.data %>% dplyr::select(SiteUnit, Order, Species, MeanCov) %>% 
...
BEC.missed <- predict(BEC_ranger_model, BEC_good) %>% bind_cols(BEC_good %>% dplyr::select(SiteUnit, Order)) %>% 
...
Hier.update <- left_join(SUhier, newBEC) %>% mutate(Order = ifelse(!is.na(.pred_class), .pred_class, Order)) %>%
        dplyr::select(-.pred_class) %>% filter(!Order == "")
