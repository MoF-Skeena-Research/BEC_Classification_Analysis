---
title: "Review of LMH 77 Grasslands"
...
taxon.all <- read_sppmaster("D:/OneDrive/BCSpeciesList/SpeciesTaxonomyMasterjuly2025.accdb")
...
su <- dbReadTable(master_su, "Coast_Grasslands_final_SU")
#su <- dbReadTable(master_su, "Coast_Grasslands2_SU")
...
unit.choose <- unique(su$SiteUnit)
 # unit.choose <- c("Bb01", "Bb01.2", "Bb02.1", "Bb02.2", "Bb02.3", "Bb02.4", "Bb03", "Bb04")
su.choose <- su %>% filter(SiteUnit %in% unit.choose)
table.name1 <- "Grassland1"
...
# 
#   unit.choose <- c("Bb05", "Bb06", "Bb07", "Bb08.1", "Bb08.2", "Bb09", "Bb10", "Bb11") 
#  su.choose <- su %>% filter(SiteUnit %in% unit.choose)
# table.name2 <- "Beach2"
# vegSum <- create_veg_sum_all(vdat = veg.dat2, siteUnits = su.choose, minimportance = 0, minconstancy = 50, noiseconstancy = 25)
# ##determine order of species by unit
...
saveWorkbook(vegsum.wbk, "./vegsum.tables/LMH77_Grasslands_table.xlsx", overwrite = TRUE)
...
#| warning: false
#| tbl-align: "left"
# ss_x_bgc <- su %>% select(SiteUnit, bgc) %>% groupû_by(SiteUnit, bgc) %>% summarise(n = n()) %>% pivot_wider(names_from = SiteUnit, values_from = n, values_fill = 0) %>% ungroup() %>%  mutate_if(is.numeric, as.character) %>% arrange(bgc)
# ss_x_bgc[ss_x_bgc == 0] <- "-"
# # low.diff <- gt::as_gtable((gt::gt(ss_similar) |> gt::fmt_number(decimals = 2)|> gt::tab_options(table.font.size = 10) |> gt::cols_width(Site.Units ~ gt::px(250))), plot = TRUE, text_grob = gridtext::richtext_grob)
