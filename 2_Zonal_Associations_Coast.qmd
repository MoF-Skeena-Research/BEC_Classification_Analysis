...
su <- combined_su(db)
su <- su %>%
  filter(!grepl('101a.2|101b.2|low|-S|add|nudum|poor|yc|_W|_F|ys|moved|X|unplaced|omit|support', SiteUnit))  %>% arrange(SiteUnit)#
...
xx <- fread("zonal_cluster_memberships.csv")


## modify this file to hard code site series group membership
```
...
hierarchy[zonal.groups, Association := assigned_assoc, on = "SiteUnit"] 

# hierarchy <- as.data.frame(hierarchy) %>% mutate(assoc_method = ifelse(SiteUnit %in% zonal.groups$SiteUnit, "Zonal", assoc_method)) %>% as.data.frame
# fwrite(hierarchy, "./hierarchy/coast_siteunits_hierarchy_membership.csv")
# 
# su_updated <- assign_manual_cluster(su.tab = su2, cluster.file = hierarchy,
...
author <- "test.assoc2"
su_zonal2 <- su_zonal %>% filter(grepl('101', SiteUnit)) #%>% select(-bgc)
...
assoc.list <- su_zonal2 %>% filter(!is.na(Step1_Group)) %>%  select(SiteUnit, Step1_Group) %>% distinct() %>% group_by(Step1_Group) %>% mutate(n=n()) %>% filter(n>=1) %>%  distinct(Step1_Group) %>% pull(Step1_Group)
#assoc.choose = "Step3_Grp1"
vegsum.wbk <- createWorkbook()
#for(assoc.choose in assoc.list){
#assoc.choose = "assoc-13"
# assoc <- su_zonal2 %>% filter(Step1_Group %in% assoc.choose)# %>% select(SiteUnit, Step1_Group) %>% mutate(SiteUnit = gsub(" ", "", SiteUnit)) %>% mutate(SiteUnit = str_replace(SiteUnit, "/", "_"))
# su.assoc <- left_join(assoc, su, by = "SiteUnit") %>%  mutate(SiteUnit = str_replace(SiteUnit, "b$|a$", "")) %>% distinct()
veg.dat.assoc <- veg.dat2 %>% filter(PlotNumber %in% su_zonal2$PlotNumber)
# vegsum.wbk <- createWorkbook()
# #bgc.choose = "CDFmmtab2"
# #for(bgc.choose in bgc.list){
vegSum <-
  create_veg_sum_all(vdat = veg.dat.assoc, siteUnits = su_zonal2, strata.by = "Lifeform")

#indic.order <- build_species_ordering_hier(vdat = veg.dat.assoc, vsum = vegSum,  siteUnits = su_zonal2, hier.level = "Step1_Group")
...
saveWorkbook(vegsum.wbk, paste0("./vegsum.tables/LMH77_zonal_assoc_tables_v4_", author, ".xlsx"), overwrite = TRUE)
...
assoc.test <- su_zonal %>% left_join(plot.eda, by = "PlotNumber")  %>% 
  drop_na(edatopic) %>%
  mutate(aSMR.code = factor(aSMR.code, levels = c("XD", "ED", "VD", "MD", "SD", "F", "M", "VM", "W", "VW"), ordered = TRUE)) %>% 
  mutate(Unit = as.factor(SiteUnit))
