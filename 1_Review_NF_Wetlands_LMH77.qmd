---
title: "Review of LMH 77 Wetlands"
...
lump <- dbReadTable(veglump, "WetlandsII_Lump")
...
su <- dbReadTable(master_su, "Coast_Wetlands_final_SU")
...
su <- su %>%
  filter(!grepl('poor|low|-S|add|nudum|browsed|X|!|support|Unplaced|remove|unplaced', SiteUnit)) %>% select(-SiteUnit.orig) %>% arrange(SiteUnit)
...
#unit.list <- unique(su$SiteUnit)
 unit.choose <- c("Wf50", "Wf51", "Wf52", "Wf52.1", "Wf52.2",  "Wf53", "Ws50", "Ws51.1", "Ws51.2")
...
unit.choose <- c("Wm04", "Wm05", "Wm06", "Wm09", "Wm50.1", "Wm50.2", "Wm50.3", "Wm51", "Wm52", "Wm53") 
 su.choose <- su %>% filter(SiteUnit %in% unit.choose)
table.name2 <- "Marshes"
vegSum <- create_veg_sum_all(vdat = veg.dat2, siteUnits = su.choose, minimportance = 0, minconstancy = 50, noiseconstancy = 25)
##determine order of species by unit
...
unit.choose <- c("Ww01", "Ww02", "Ww03", "Ww04", "Ww05", "Ww06") 
 su.choose <- su %>% filter(SiteUnit %in% unit.choose)
table.name3 <- "Shallow waters"
vegSum <- create_veg_sum_all(vdat = veg.dat2, siteUnits = su.choose, minimportance = 0, minconstancy = 50, noiseconstancy = 25)
##determine order of species by unit
indic.order <- build_species_ordering_all(vdat = veg.dat2, vsum = vegSum, code.lump=lump, siteUnits = su.choose)
...
saveWorkbook(vegsum.wbk, "./vegsum.tables/LMH77_draft_fenmarsh_tables.xlsx", overwrite = TRUE)
