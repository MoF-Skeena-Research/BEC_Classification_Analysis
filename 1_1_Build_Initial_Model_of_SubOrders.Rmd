...
BEC.pred <- predict(BEC_ranger_model, BEC_good) %>% bind_cols(BEC_good %>% dplyr::select(SiteUnit, Suborder)) #%>%    mutate(Compare = ifelse(.pred_class == Suborder, "Same", "Diff")) %>% filter(Compare == "Diff")
...
fwrite(testReverse, "./outputs/FunctionTest_updated.csv")
...
fwrite(testReverse2, "./outputs/UpdatedVPROHierarchyv12_updated.csv")
...
#class.dat2 <- class.dat %>% select(-SiteUnit)
BEC_good2 <- class.dat2 %>% filter(!is.na(Suborder)) %>% arrange(SiteUnit)
...
BEC.mis.list2 <- BEC.missed$SiteUnit %>% as.character
