...
#   rename(id = V1) %>% na.omit() 
#remove NAs
all_points <- fread("./ESSF_climatePts.csv") %>% as_tibble() 
my_points <- all_points %>% select(lat, lon = long, elev,id) %>% na.omit()
...
clim_dat <- downscale(
  xyz = my_points, 
 vars = list_vars(set = c("Annual", "Seasonal")))
toc()
grid.dat<-left_join(clim_dat, all_points)
...
#     .names = "{.col}_{.fn}"
#   )) %>%  select(BGC, MAT, DD5_an, MSP, PAS, MWMT, MCMT)
vars <- c("MAT", "DD5_an", "MSP", "PAS_an", "MWMT", "MCMT")

grid.dat.long <- grid.dat %>%
  pivot_longer(
    cols = all_of(vars),
    names_to = "variable",
    values_to = "value"
...
ggplot(grid.dat.long, aes(x = BGC, y = value)) +
  geom_boxplot() +
  facet_wrap(~ variable, scales = "free_y") +
  labs(
    title = "Climate Variable Distributions by BGC",
    x = "BGC",
    y = "Value"
  ) +
  theme_minimal() +
