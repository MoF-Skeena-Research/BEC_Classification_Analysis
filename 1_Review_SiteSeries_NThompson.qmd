---
title: "Review of NThompson Guide Site Series by BGC"
...
source('../BEC_R_working/_functions/_create_veg_sum_bgc.R')
...
Vegetation data is read in from saved .RDS file generated from the BECMaster cleaning scripts. A compiled SU table is build from all BGC \_SU tables stored in the guide's SU database. Taxonomy is read in from the species taxonomy database. A species lumping code table is read in from the Correlation2_Spp_lump.accdb database and the vegetation data is lumped using the lump_species function.
...
db <- "C:/Users/whmacken/Dropbox/NorthThompson_LMH/NorthThompson_FG_Sorts.accdb"
su <- combined_su(db)
su.nf <- su %>%
...
```{r build pair.wise similarity matrix, echo=FALSE, messages = FALSE}

key.site.indicators <- c("PICESIT", "POPUTRI", "LYSIAME", "GAULSHA", "OPLOHOR", "ELLIPYR","ATHYFIL", "RUBUSPE", "EQUIARV", "GYMNDRY", "VALESIT", "CASSMER", "LUETPEC", "POLYMUN", "STRUSPI", "RHODGRO", "EMPENIG", "NEPHCRI", "CALTLEP", "CALTBIF", "SENETRI")#,  "TIARELLA", "DRYOEXP" )
...
vegsum.pairs <- do_pairwise(veg.dat2,
  su = su2, minimportance = 0, minconstancy = 50,  ### change this to 50 reduce up to 60
  noiseconstancy = 10,
  minplots = 1,
  minor = 1,
...
### Site Units with few plots

Units with fewer than 6 plots are often problematic for quantitative analysis
...
### Site Units with low diagnostic potential

Often a sign of poor cohesion of plots and plot set should be reviewed
...
### Site Units in BGC that are very similar

These are units are unlikely to separate at a site series level and should reviewed or considered for phases of a single site series
...
The dendrograms show graphically how different the site series within a BGC are with the purpose of identifying units that do not differentiate adequately and require review as well as show likely within BGC association and alliance level similarities

## Dendrogram of cluster analysis by BGC

The dendrogram is an approximate representation of the similarity matrix. It is constructed using agglomerative hierarchical clustering which merges site units from the bottom up. The red line represents the minimal dissimilarity required to separate units into different site series. Any splits to the right of the red line should be considered for merging into the same site series (possibly as phases). Current threshold is set at 7% but assessment is required. Splits that occur between the green association threshold and the red minimum threshold will likely be merged at the subassociation level in the hierarchy. The green line represents the dissimilarity threshold for an association. Site unit 'leaves' to the right of this threshold will fall under the same association in the hierarchy. Current threshold is set at 17% but assessment is required. The cophonetic value is an indication of how well the dendrogram can maintain the similarity order from the matrix. Values above 0.70 are considered good.

```{r cluster analysis, echo=FALSE}
#bgc.unique <- c("CDFmm", "CWHvm3")
bgc.unique <- unique(su2$bgc[startsWith(su2$bgc, "CWH")])

bgc.unique <- unique(su2$bgc)
# singles <- for (bgc.choose in bgc.unique){
#   create_dendro_bybgc(bgc.choose, unit.compare, threshold.low = .07, threshold.high = .15)
# }
singles <- lapply(
  bgc.unique,
  function(bgc.choose) {
...
library(dplyr)
singles.tables <- bind_rows(lapply(singles, `[[`, "similar_units"))

fwrite(singles.tables, "./hierarchy/within_bgc_clusters.csv")
...
veg.dat2 <- combine_taxa_strata(vegdata = vegdata, lump, use.subtaxa = FALSE)
...
vegSum <- 
  create_veg_sum_bgc(vegdata = veg.dat2, siteunit.tbl = su3 , BGC = bgc.choose, minconstancy = 50, noiseconstancy = 25, minimportance = 0, strata.by = "Layer")
...
veg.sum.table <- format_veg_table_strata(vsum = vegSum, spp=taxon.lifeform)
...
This output is to help in review of sufficient edatopic separation between proposed site units and for drafting up fieldguide grids.

- Any ½ class ratings  get ignored in the code (e.g SMR4-, or SMR4+ become SMR4)

- If there are Null or invalid values for either the SMR or SNR then no info from a plot gets used.

```{r build edatopic table, echo=FALSE, message=FALSE}

#| fig-width: 8.5
#| fig-height: !expr (ceiling(length(unique(su$SiteUnit[su$bgc == b])) / 3)) * 3
#| fig.show: hold
#bgc.unique <- "ESSFwc8"
...
for (b in bgc.unique) {
  print(draw_ss_edatope(plot.env, su2, b))
  #cat("\n\n\\newpage\n\n")
}
...
# for (b in bgc.unique) {
# 
#   p <- draw_ss_edatope(plot.env, su, b)
# 
#   # ✅ print explicitly
#   print(p)
# 
#   # ✅ force separate figure
#   cat("\n\n\\newpage\n\n")
# }

# ✅ If it STILL overlaps (rare but possible)

# Wrap the plot in a **grid draw wrapper**:
# for (b in bgc.unique) {
# 
#   grid::grid.newpage()
#   grid::grid.draw(draw_ss_edatope(plot.env, su, b))
# 
#   cat("\n\n\\newpage\n\n")
# }
```

```{r Generate formatted table summary report for BGC, echo=FALSE}
