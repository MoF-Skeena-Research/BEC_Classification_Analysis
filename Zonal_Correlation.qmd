...
Works reasonably well in identifying lower groups but sometimes it seems like subzone group branches are misassigned to expected higher groupings.
Generally the clustering matches the expected groupings when tree species and other key climate indicators are emphasized in the analysis.
...
Produce output table for review where units have low diagnostic potential (usually a function of heterogenous units in forest types) or high variability in plot membership
