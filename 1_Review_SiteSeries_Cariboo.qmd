---
title: "Review of Cariboo Region Site Series by BGC"
...
su <- su %>%
  filter(!grepl('poor|low|-S|add|nudum|_[[:alpha:]]|X', SiteUnit)) %>% select(-SiteUnit.orig) %>% arrange(SiteUnit)
#   filter(grepl('01', SiteUnit)) ###zonal specific
su2 <- su 
#su2 <- su %>% filter(bgc %in% c('IDFdw'))
author = "KIverson"
xx <- su2 %>% filter(SiteUnit == "IDFdk3_03")
xx <- left_join(xx, veg.dat2) #%>% filter(SiteUnit == "ESSFwc3_110")
###BGC specific|CWHms
##AB units
# su2 <- su2 %>%
...
key.site.indicators <- c("PICESIT", "POPUTRI", "LYSIAME", "GAULSHA", "OPLOHOR", "ELLIPYR","ATHYFIL", "RUBUSPE", "EQUIARV", "GYMNDRY", "VALESIT", "CASSMER", "LUETPEC", "POLYMUN", "STRUSPI", "RHODGRO", "EMPENIG", "NEPHCRI", "CALTLEP", "CALTBIF", "CALARUB")#,  "TIARELLA", "DRYOEXP" )
reduced.exceptions <- c("SPHAGNUM", "CLADONIA", "CLADINA", "RACOMITR", "MNIUM")
# reduced.lifeforms = c(1,2)
reduced.lifeforms <- c(9, 10, 11)
### select units to run

vegsum.pairs <- do_pairwise(veg.dat = veg.dat2,
...
xx <- vegsum.pairs %>%
  filter(Unit2 == "IDFdk3_03")#, Unit2 == "CWHds1_101b")
...
# 
# veg.dat.lump <- lump_species2(vegdata = vegdata, lump, use.subtaxa = FALSE)
# BGC <- "MHmm1"
# for(bgc.choose in bgc.unique){
#   vegSum <-
#   create_veg_sum(vdat = veg.dat.lump, siteUnits = su , BGC = bgc.choose, strata.by = "Layer")
# ##determine order of species by unit
# indic.order <- build_species_ordering(vdat = veg.dat.lump, vsum = vegSum, code.lump=lump, siteUnits = su, BGC = bgc.choose)
...
# for(bgc.choose in bgc.list){
# vegSum <- 
#   create_veg_sum_bgc(vdat = veg.dat2, siteUnits = su2 , BGC = bgc.choose, minconstancy = 50, noiseconstancy = 25, strata.by = "Layer")
# ##determine order of species by unit
...
# }
# toc()
# saveWorkbook(vegsum.wbk, paste0("./vegsum.tables/Cariboo_draft_veg_tables_", author, ".xlsx"), overwrite = TRUE)
