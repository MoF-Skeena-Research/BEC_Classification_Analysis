...
db <- "D:/BC_Correlation2_Vpro_2023/Cariboo_SUs.accdb"
su <- combined_su(db)
su.nf <- su %>%
...
xx <- left_join(su2, veg.dat2) %>% filter(SiteUnit == "ESSFwc3_110")
###BGC specific|CWHms
##AB units
# su2 <- su2 %>%
...
for(bgc.choose in bgc.list){
vegSum <- 
  create_veg_sum(vdat = veg.dat2, siteUnits = su2 , BGC = bgc.choose, minconstancy = 50, noiseconstancy = 25, strata.by = "Layer")
##determine order of species by unit
