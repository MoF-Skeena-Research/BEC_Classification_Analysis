...
#su <- su %>% filter(!grepl('CWHvh3|CWHwh1|CWHwh2|CWHvh3|MHwh', SiteUnit))###BGC specific|CWHms
...
guide.vars <- coast.climr %>% select(BGC_2, MAP, MSP, PAS, MAT, MWMT, MCMT, DDsub0, DD5, FFP, NFFD, EXT, EMT, CMD.total ) %>% filter(BGC_2 %in% bgc.unique$bgc)
guide.clim.sum <- guide.vars %>% group_by(BGC_2) %>% summarise_all(list( min=min, mean=mean, median=median, max=max, sd=sd, IQR = IQR), na.rm = TRUE) %>% mutate(n = n()) %>% dplyr::relocate(sort(names(.)))
