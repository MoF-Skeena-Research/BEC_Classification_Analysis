...
source("../BEC_R_working/_functions/_combine_SU_databases.R")
source("../BEC_R_working/_functions/_buildSU.R")
```

#### Import becdb site series
```{r set folders, include=FALSE}
require(DBI) #loads odbc as well
 becdb <- dbConnect(odbc::odbc(), .connection_string = "Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=D:/OneDrive - Government of BC/BECdb_Working/BECdb_ver14_2026_v0.accdb;")

BGCs <- dbReadTable(becdb, "tblBGC_UnitsWorkingandArchive") %>% filter(Status == "Current") %>% select(BGC_Label, BGC_NoSpace, ZONE) 
siteseries_all<- dbReadTable(becdb, "tblBEC_SiteSeriesWorkingandArchive") %>% filter(!Status == "Retired") %>% select(BGC_NoSpace, SS_NoSpace, SU_Label, Forest_NonForest) 
ss_v13_all <- siteseries_all %>% pull(SU_Label)
siteseries_for <- siteseries_all %>% filter(!Forest_NonForest %in% c("NonForest", "NonForestSeral")) #c("Forest", "Treed")) 
ss_v13_for <- siteseries_for %>% pull(SU_Label)
# dbDisconnect(becdb)
BGCs_noSS <- anti_join(BGCs , siteseries_all) %>% drop_na
# 
# becdb <- dbConnect(odbc::odbc(), .connection_string = "Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=D:/BECdb_Working/BECdb_ver14_2026_v0.accdb;")
dbWriteTable(becdb, "BGCs_NoSiteClassification",  BGCs_noSS, overwrite = TRUE, batch_rows = 1)
dbDisconnect(becdb)
```

#### Import all BECMaster data tables in /raw_data folder
```{r set folders, include=FALSE}
require(DBI) #loads odbc as well
...
Skip this if it is already created with latest sorts
```{r function to create provincial SU table}
library(odbc)
library(dplyr)
library(stringr)

## Combine _SU tables from all databases under folder_path
folder_path <- "D:/BC_Correlation2_Vpro_2023/Sorts_Regional_BGC/"  # Path to folder containing .accdb files
SU_all <- combine_SU_databases(folder_path)
...
duplicates <- SU_all %>%
  ...
  arrange(PlotNumber) 
plots_per_SU <- SU_all %>%
  distinct(PlotNumber, SiteUnit) %>%
  count(SiteUnit, name = "n_plots") %>%
  arrange(desc(n_plots))


plots_per_SS <- SU_all %>% 
  mutate(SiteUnit = str_replace(SiteUnit, "([abc]|\\.\\d+)$", "")) %>% 
  distinct(PlotNumber, SiteUnit) %>%
  count(SiteUnit, name = "n_plots") %>%
  arrange(desc(n_plots))

plots_per_SU <- bind_rows(plots_per_SS, plots_per_SU) %>% distinct

# Sort for clarity
## Write into Correlation database
...
dbWriteTable(master_su, "nplots_per_siteunit",  plots_per_SU, overwrite = TRUE, batch_rows = 1, row.names = FALSE)
dbDisconnect(master_su)
fwrite(SU_all, "./clean_tabs/All_BECv13_published_SU.csv")
## for updating the plot count in the tbl_BEC_Hierarchy_Class
 becdb <- dbConnect(odbc::odbc(), .connection_string = "Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=D:/OneDrive - Government of BC/BECdb_Working/BECdb_ver14_2026_v0.accdb;")
dbWriteTable(becdb, "nplots_per_siteunit",  plots_per_SU, overwrite = TRUE, batch_rows = 1, row.names = FALSE)
dbDisconnect(becdb)
# update the BEC
# View output
#head(SU_all)
...
```
## check for missing SUs in published LMH SU table
```{r set folders, include=FALSE}
###Add in nplots per site unit
SU <- dbConnect(
  odbc::odbc(),
  .connection_string = "Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=D:/BC_Correlation2_Vpro_2023/All_BC_Correlation.accdb;")
sorts.LMH <- dbReadTable(SU, "All_BECv13_published_SU")
sorts.LMH2 <- left_join(sorts.LMH, siteseries_all, by = c("SiteUnit" = "SU_Label"))
dbDisconnect(SU)
# Ensure both sides are character vectors
su_all <- sorts.LMH %>% pull(SiteUnit)
su_for <- sorts.LMH2 %>% pull(SiteUnit)
# Find SUs in sorts.LMH that are missing from the v13 site-series table
missing_su <- setdiff(su_all, ss_v13_all) %>% as.data.frame()
missing_su

missing_ss <- setdiff(ss_v13_all, su_all) %>% as.data.frame()
missing_ss
