...
SU <- fread("./clean_tabs/CoastGuide_All_BGC_Forest_SS.csv")
```
...
veg_anal <- create_analysis_vegsum (vegsum, importance = 0.5, constancy = 60, minplots = 0) 
...
Hier.units <- Hier.clean %>% dplyr::select(SiteUnit, Class) %>% distinct()
Hier.data <- left_join(Hier.units, veg_anal)  %>% arrange(Species) %>% distinct() %>% mutate(Class = ifelse(is.na(Class), "unplaced", Class))
```
...
class.dat <-
  Hier.data %>% dplyr::select(SiteUnit, Class, Species, spp_importance) %>% mutate(Species = ifelse(is.na(Species), "None", Species)) %>% 
  pivot_wider(id_cols = c(SiteUnit, Class),
    names_from = Species,
    values_from = spp_importance) %>% distinct() %>% dplyr::select(-None) %>% droplevels() #%>% mutate(None = ifelse(is.numeric(complete.cases(.)), 1, 0))
...
classID <- class.dat %>% dplyr::select(SiteUnit, Class)
#class.dat2 <- class.dat %>% select(-SiteUnit)
BEC_good <- class.dat %>% filter(!Class == "unplaced") %>% arrange(SiteUnit)
...
BEC_new <- class.dat %>% filter(Class == "unplaced") %>% arrange(SiteUnit)
...
save(BEC_ranger_model, file = "./rFmodels/BECv12_Classes_rFmodel.Rdata")

BEC.pred <- predict(BEC_ranger_model, BEC_good) %>% bind_cols(BEC_good %>% dplyr::select(SiteUnit, Class)) 


# %>%  metrics(truth = Class), estimate = .pred_class)
BEC.pred.list <- BEC.pred$SiteUnit %>% as.character
BEC.missed <- BEC.pred  %>%    mutate(Compare = ifelse(.pred_class == Class, "Same", "Diff")) %>% filter(Compare == "Diff")
...
BEC.new.pred <- predict(BEC_ranger_model, BEC_new) %>% bind_cols(BEC_new %>% dplyr::select(SiteUnit, Class)) %>% dplyr::rename(Class_updated = .pred_class)
