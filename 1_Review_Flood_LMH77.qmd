---
title: "Review of LMH 77 Flood Forests"
...
Vegetation data is read in from saved .RDS file generated from the BECMaster cleaning scripts. A compiled SU table is build from all BGC SU tables stored in the coast guide Vpro database. Taxonomy is read in from the species taxonomy database. A species lumping code table is read in from the Correlation2_Spp_lump.accdb database and the vegetation data is lumped using the lump_species function.
...
su <- dbReadTable(master_su, "Coast_Flood_Final_SU")
...
#for (bgc.choose in bgc.unique){
  create_dendro_all(unit.compare, threshold.low = .07, method = "average", parameter = .9)
#}
```
...
table.name1 <- "Flood1"
# unit.choose <- c("Ws51", "Ws52", "Ws53", "Ws56.1", "Ws56.2", "Ws56.3",  "Ws57")
...
saveWorkbook(vegsum.wbk, "./vegsum.tables/LMH77_Flood_Associations.xlsx", overwrite = TRUE)
