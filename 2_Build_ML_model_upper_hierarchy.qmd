...
```{r import data and create veg summary}
...
su <- combined_su(db)
SUTab <- su %>%
  filter(!grepl('101a.2|101b.2|low|-S|add|nudum|poor|yc|ys|moved|X|unplaced|support|omit', SiteUnit))  %>% arrange(SiteUnit)#_W|_F|
...
vegsum.all<- create_su_vegdata_orders(veg.dat, SUTab)

veg.dat2 <- veg.dat %>% filter(Lifeform %in% c("1", "2"))
vegsum.trees <- create_su_vegdata_orders(veg.dat2, SUTab)

spp.rain <- c("TSUGHET", "PICESIT")
spp.coast.high <- c("TSUGMER", "CALLNOO", "ABIELAS")
coast.dry <- c("PSEUMEN2")
coast.montane <- c("ABIEAMA")
coast.s <- c("THUJPLI", "ABIEGRA")
decid.coast <- c("ALNURUB", "ACERMAC", "MALUFUS", "QUERGAR", "POPUTRI")

vegsum.trees <- vegsum.trees %>%
   mutate(Spp.group = case_when(
     Species %in% spp.rain ~ "CWH",
     Species %in% coast.s ~ "CWHs",
     Species %in% coast.dry ~ "Fdc",
      Species %in% spp.coast.high ~ "MH",
      Species %in% coast.montane ~ "Ba",
      Species %in% decid.coast ~ "Coastal_Deciduous",
     Species == "PSEUMEN1" ~ "Fdi",
      Species == "PINUCON1" ~ "Plc",
     TRUE ~ "Other"
   ))
```


## Create rule-based tree-based Orders
```{r summarize site series, echo=FALSE}
spp.groups <- vegsum.trees %>%
  group_by(SiteUnit, Spp.group) %>%
  summarise(group_cover = sum(MeanCov, na.rm = TRUE), .groups = "drop") %>%
  group_by(SiteUnit) %>%
  mutate(
    total_cover = sum(group_cover),
    group_prop = group_cover / total_cover
  )
siteunit.groups <- spp.groups %>%
  summarise(
    MH_Ba = sum(group_prop[Spp.group %in% c("MH", "Ba")], na.rm = TRUE),
    MH = sum(group_prop[Spp.group == "MH"], na.rm = TRUE),
    Fdc = sum(group_prop[Spp.group %in% c("Fdc", "CWHs")], na.rm = TRUE),
    Decid = sum(group_prop[Spp.group == "Coastal_Deciduous"], na.rm = TRUE),
    CWH = sum(group_prop[Spp.group %in% c("CWH", "CWHs")], na.rm = TRUE),
    Fdi = sum(group_prop[Spp.group == "Fdi"], na.rm = TRUE),
    Bog = sum(group_prop[Spp.group == "Plc"], na.rm = TRUE)
  ) %>%
  mutate(
    anal_group = case_when(
      MH_Ba > 0.70 ~ "MH",
      MH > 0.50 ~ "MH",
      #Fdc > 0.90 ~ "Fdc",
      Decid > 0.7 ~ "Deciduous",
...
anal.groups <- siteunit.groups %>% select(SiteUnit, anal_group) %>% distinct
...
## Prep two data sets 
one with trees only and 
one with all species
```{r prep data, include = TRUE, echo = FALSE}
#vegsum.all<- create_su_vegdata_orders(veg.dat, SUTab)
tree.data <- left_join(anal.groups, vegsum.trees) %>% filter(!is.na(nplots)) %>% arrange(Species) %>% distinct()
tree.sum <-
  tree.data %>% dplyr::select(SiteUnit, anal_group, Species, MeanCov, Constancy) %>% 
  pivot_wider(id_cols = c(SiteUnit, anal_group),
    names_from = Species,
    values_from = c(MeanCov, Constancy)) %>% 
  mutate(Order = ifelse(is.na(anal_group) | anal_group == "", "unplaced", anal_group)) %>% filter(!SiteUnit == "") %>%  mutate_if(is.character, as.factor) %>% 
   replace(is.na(.),0) %>% distinct() %>% droplevels()%>% select(-anal_group)
#classID <- class.dat %>% dplyr::select(SiteUnit, anal_group)
all.data <- left_join(anal.groups, vegsum.all) %>% filter(!is.na(nplots)) %>% arrange(Species) %>% distinct()
all.sum <-
  all.data %>% dplyr::select(SiteUnit, anal_group, Species, MeanCov, Constancy) %>% 
  pivot_wider(id_cols = c(SiteUnit, anal_group),
    names_from = Species,
    values_from = c(MeanCov, Constancy)) %>% 
  mutate(Order = ifelse(is.na(anal_group) | anal_group == "", "unplaced", anal_group)) %>% filter(!SiteUnit == "") %>%  mutate_if(is.character, as.factor) %>% 
...
## Build an ML model with tree cover and constancy
...
```{r examine ranger model}
set.seed(123)
BEC_split <- initial_split(tree.sum, strata = Order, prop = .75)
...
# the last fit
set.seed(345)
BEC_ranger_model_trees <- fit(BEC_workflow, BEC_train)
BEC_ranger_model_trees

BEC_recipe <-
    recipe(Order ~ ., data = tree.sum) %>%
     update_role(SiteUnit, new_role = "id variable") %>% 
  step_novel(SiteUnit)
BEC_ranger_model_trees <- fit(BEC_workflow, tree.sum)
save(BEC_ranger_model_trees, file = "./rFmodels/Coast_Orders_trees_rFmodel.Rdata")


BEC.pred <- predict(BEC_ranger_model_trees, tree.sum) %>% bind_cols(tree.sum) %>% dplyr::select(SiteUnit, Order, .pred_class) 
...
hierarchy[BEC.pred, Order := .pred_class, on = "SiteUnit"] 
...
```{r examine ranger model}
#BEC.pred <- predict(BEC_ranger_model_trees, BEC_test) %>% bind_cols(BEC_test %>% dplyr::select(SiteUnit, anal_group)) 
tree.sum2 <- tree.sum %>% select(-SiteUnit) %>% mutate(Order = as.factor(tree.sum$Order)) # to avoid factor conversion issues
C5_model <- C5.0(x = tree.sum2 %>% select(-Order), y = tree.sum2$Order)

plot(C5_model)         # Basic plot (needs R graphics device)
summary(C5_model) 

library(randomForestExplainer)


# Make sure you have a standard ranger/randomForest object
representative_tree <- min_depth_distribution(ranger_model)
plot_min_depth_distribution(representative_tree)

library(rpart)
library(rpart.plot)

tree_model <- rpart(Order ~ ., data = tree.sum2, method = "class")

rpart.plot(tree_model, type = 4, extra = 101, box.palette = "Greens", under = TRUE)

```


```{r pull example ranger model}
ranger_model <- BEC_ranger_model_trees$fit$fit$fit
class(ranger_model)
tree_df <- ranger::treeInfo(ranger_model, tree = 5)
library(data.tree)
library(DiagrammeR)

# Create root node
forest_tree <- Node$new("root2")
forest_tree$label <- "Root2"  # Optional: label root node

# Recursive tree builder with explicit labeling
add_branch <- function(node, df, id) {
  row <- df[df$nodeID == id, ]

  if (row$terminal == TRUE) {
    # Collect pred.* columns and format as readable string
    prob_cols <- grep("^pred\\.", names(row), value = TRUE)
    probs <- round(as.numeric(row[prob_cols]), 2)
    prob_labels <- paste0(gsub("pred\\.", "", prob_cols), ": ", probs, collapse = "\n")
    node$label <- paste("Leaf →", row$prediction, "\n", prob_labels)
  } else {
    split_var <- row$splitvarName
    split_val <- row$splitval
# Format numeric values nicely, pass others unchanged
if (is.numeric(split_val)) {
  split_val <- round(split_val, 2)
}

# Create left child (<=) with non-empty name
left <- node$AddChild("left")
left_label <- paste(split_var, "<=", split_val)
left$label <- left_label
add_branch(left, df, row$leftChild)

# Create right child (>) with non-empty name
right <- node$AddChild("right")
right_label <- paste(split_var, ">", split_val)
right$label <- right_label
add_branch(right, df, row$rightChild)
  }
}

# Build the tree starting from node 0
add_branch(forest_tree, tree_df, id = 0)

# Ensure DiagrammeR uses your labels
forest_tree$Do(function(node) {
  node$name <- node$label
})

# Render
ToDiagrammeRGraph(forest_tree) %>%
  DiagrammeR::render_graph()


```
## Now build model with all species

```{r final ranger model}
BEC_recipe <-
    recipe(Order ~ ., data = all.sum) %>%
     update_role(SiteUnit, new_role = "id variable") %>% 
  step_novel(SiteUnit)
BEC_workflow <- workflow() %>%
  add_model(BEC_fmodel) %>% 
...
BEC_ranger_model_all <- fit(BEC_workflow, all.sum)
BEC_ranger_model_all
save(BEC_ranger_model_all, file = "./rFmodels/Coast_Orders_allspp_rFmodel.Rdata")

# all.sum2 <- all.sum %>% select(-SiteUnit) %>% mutate(Order = as.factor(all.sum$Order)) # to avoid factor conversion issues
# C5_model <- C5.0(x = all.sum2 %>% select(-Order), y = all.sum2$Order)
# 
# plot(C5_model)         # Basic plot (needs R graphics device)
# summary(C5_model) 
```
