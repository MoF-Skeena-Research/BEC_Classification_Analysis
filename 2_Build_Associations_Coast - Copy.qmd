...
#db <- "D:/BC_Correlation2_Vpro_2023/Sorts_Regional_BGC/CoastArea_Sorts.accdb"
#db <- "D:/BC_Correlation2_Vpro_2023/Coast_Association_Exercise.accdb"
su <- combined_su(db)

db <- "D:/BC_Correlation2_Vpro_2023/Sorts_published_LMH/LMH68_HG_Sorts.accdb"
su2 <- combined_su(db)
su <- rbind(su, su2)

su <- su %>%
  filter(!grepl('X|unplaced|omit|support|nudum|Ww|!', SiteUnit))  %>% arrange(SiteUnit)#
n_plots <- su %>% group_by(SiteUnit.orig) %>% summarise(n_plots = n_distinct(PlotNumber)) %>% rename(SS_Label = SiteUnit.orig)
...
#remove = c( "00", "/x")#"Gg", "Ro", "Gs", "Rt", "Gb",, "Bb", "Br", "/Fm", "Fl"
...
dbWriteTable(
  becdb,
  "n_plots",
  n_plots, append=FALSE,
  overwrite = TRUE,
  batch_rows = 1
)
BGC_all <- dbReadTable(becdb, "tblBGC_UnitsWorkingandArchive") %>% 
  filter(is.na(VersionRetired)) %>% select(FSRegion, BGC_NoSpace)
BEC.hier <- dbReadTable(becdb, "tblBEC_Hierarchy_Class") %>% filter(SS_Label %in% su2$SiteUnit.orig) %>% #select(SS_Label, Subass, Association, Suball, Alliance, Subord, Order, Class) %>% 
  #filter(!str_detect(SS_Label, paste(remove, collapse = "|"))) %>% 
    mutate(
    SiteUnit = str_replace_all(SS_Label, c("/" = "_", " " = "")))
#mutate(SiteUnit = gsub("/", "_", SS_Label))
bgcv13 <- dbReadTable(becdb, "tblBGC_UnitsWorkingandArchive")%>% filter(is.na(VersionRetired)) %>% select(BGC_NoSpace, SiteClassificationStatus)
...
###SU not in hierarchy membership file
missing_in_hierarchy <- su2 %>%
  distinct(SiteUnit) %>%
  anti_join(BEC.hier %>% distinct(SiteUnit), by = c("SiteUnit")) %>%
...
### This chunk limits the SU and adds the hierarchy level requested (if exists) based on the table tblBEC_Hierarchy_Class. This is a working table that can be edited to hardwire SU group membership at any level of the hierarchy. The code will assign SU to the highest level of the hierarchy that has been filled in for each SU. For example, if an SU has an association filled in but no subassociation, it will be assigned to the association level. If an SU has no association but has a suballiance filled in, it will be assigned to the suballiance level. This allows for flexible grouping of SU based on the available information in the hierarchy table.
```{r choose level, echo=FALSE}
hierarchy <- c("Class", "Order", " Suborder", "Alliance", "Suballiance", 
               "Association", "Subassociation", "temp_Assoc_Code", "tempSubass_Code", "SiteUnit")
BEC.hier2 <- BEC.hier %>% select(-SiteUnit)
su.anal <- su2 %>% left_join(BEC.hier2, by = c("SiteUnit.orig" = "SS_Label")) 
anal.set = c("Forest")
group.to.test <- "Association"
su.anal <- su.anal %>% filter(Analysis_Group %in% anal.set)
xx <- su.anal %>% filter(SiteUnit == "CWHds1_101")
su.anal <- su.anal %>%
  rowwise() %>%
  mutate(
    anal.test = if (!is.na(Analysis_Group)) {
      Analysis_Group
    } else {
      # find the next lower non-null level
      current_index <- match("Analysis_Group", hierarchy)
      lower_levels <- hierarchy[(current_index + 1):length(hierarchy)]
      first_non_null <- first(na.omit(c_across(all_of(lower_levels))))
      first_non_null
    }
  ) %>%
  ungroup()%>% select(PlotNumber, SiteUnit, SiteUnit.orig, group.to.test, anal.test) %>% 
mutate(
    SiteUnit.hier = coalesce(!!sym(group.to.test), SiteUnit)
  ) %>%
  select(PlotNumber, SiteUnit, SiteUnit.orig, !!sym(group.to.test), anal.test, SiteUnit.hier)


```
Other summary tables for comparison (e.g. USA) table can be added to the analysis for comparison in do.pairwise function in the next chunk
```{r}
...
## use existing association names to group site units for the analysis where they exist
su_working <- su.anal %>% rename(SiteUnit.base = SiteUnit, SiteUnit = SiteUnit.hier ) #su_hier %>%
...
## look for additional merging to existing hierchy units
...
### find initial similar units from clusters and write into BECdb
Rerun from start to employ these new clusters and iterate.
```{r cluster analysis, echo=FALSE}
source("../BEC_R_working/_functions/_bec_dist_matrix.R")
dendro_hc <- create_dendro_all(unit.compare)
clusters <- cutree(dendro_hc, h = 0.15)
...
assoc_lookup <- su.anal[!is.na(Association), .(Association)] %>% distinct %>% pull
...
cluster_update <- merge(cluster_dt, su.anal, by = c("SiteUnit"), all.x = TRUE) %>% select(SS_Label = SiteUnit.orig,  tempAssoc) %>% distinct %>% drop_na
...
```
```{r}
new.su <- BEC.hier %>% filter(SiteUnit %in% su.anal$SiteUnit)
su.anal2 <- su.anal %>% left_join(BEC.hier) %>%  select(PlotNumber, SiteUnit, groups = Association)
```


## Create summary table for selected associations and export to excel for review and editing of membership
...
for (g in sort(unique(su.zonal$groups))) {
...
saveWorkbook(
  vegsum.wbk,
  paste0("./vegsum.tables/CWH_", author, ".xlsx"),
  overwrite = TRUE
)
...
```{r build pair.wise similarity matrix 2, echo=FALSE, message=FALSE, warning=FALSE}
